	...
	switch (ch) {
		case '\n': {
			if (p_con->cursor < p_con->original_addr + p_con->v_mem_limit - SCREEN_WIDTH) {
				LF_stack[++LF_top].prev = p_con->cursor; // 记录换行开始位置
				p_con->cursor = p_con->original_addr +
				                SCREEN_WIDTH * ((p_con->cursor - p_con->original_addr) / SCREEN_WIDTH + 1);
				LF_stack[LF_top].post = p_con->cursor; // 记录换行结束位置
				if (!p_con->rolling) {
		...
		case '\t': { // Tab，输入四个空格，保存加入四个空格后的 p_con->cursor 的值
		...
		default: {
			u8 color = DEFAULT_CHAR_COLOR; // 默认黑底白字
			if (p_con->searchMode) { // 如果是查找模式，字体颜色要变红
...
PUBLIC int rollback(CONSOLE *p_con) { // 先前已经将字符存入栈，所以这里直接调用 outchar 即可
	p_con->rolling = 1;
	int ret = 1; //
	if (operation_top >= 0) {
